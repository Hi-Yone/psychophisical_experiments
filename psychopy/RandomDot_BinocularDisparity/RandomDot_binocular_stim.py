...
# モニタサイズと視距離の定義
MONITOR_SIZE_PIX = [1440, 900]  # モニタサイズ(x, y) [pixel]
...
corrDots_r = 5            # 相関ありドットの描画半径 [deg]
patchPos = 0                # 相関ありドットパッチの中心位置
Ndots = 10000                 # ドットの生成数
disparity = 8               # 視差 [deg]
elemSize = 0.05                # ドットサイズ [deg]
...
corr_dots_Leye[:, 0] -= disparity
corr_dots_Reye = pos_dots[select_range]
corr_dots_Reye[:, 0] += disparity
...
uncorr_dots_Leye[:, 0] -= disparity
uncorr_dots_Leye -= np.random.uniform(-0.5, 0.5, (len(uncorr_dots_Leye), 2))
uncorr_dots_Reye = pos_dots[~select_range]
uncorr_dots_Reye[:, 0] += disparity
uncorr_dots_Reye += np.random.uniform(-0.5, 0.5, (len(uncorr_dots_Reye), 2))
