...
corr_dots_Leye_ref = pos_dots[select_range_test]
corr_dots_Leye_ref[:, 0] -= patch_centerPos - disparity_ref
corr_dots_Reye_ref = pos_dots[select_range_test]
...
# 周辺のドット
# ============================================================================
uncorr_dots_Leye = pos_dots[~(select_range_ref | select_range_test)]
uncorr_dots_Leye[:, 0] -= patch_centerPos
# uncorr_dots_Leye -= np.random.uniform(-0.15, 0.15, (len(uncorr_dots_Leye), 2))
uncorr_dots_Reye = pos_dots[~(select_range_ref | select_range_test)]
...
uncorrStim_Leye = visual.ElementArrayStim(
    ...
    units       = 'deg',                        # 単位 (視角) [degree]
    fieldPos    = (0, 0),                       # パッチの位置
    nElements   = len(pos_dots[~(select_range_ref | select_range_test)]), # ドット数 (ドット数は変更できないので、毎回同じドット数が表示される)
