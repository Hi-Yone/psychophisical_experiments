...
mirror1_mirror2_dist = 14                                                      # [cm]
mirror2_display_dist = 37                                                       # [cm]
...
win = visual.Window(
    ...
    screen = 2,                                                                 # スクリーンを選択 (モニタが複数ある場合に，どのモニタに表示するか)
...
fp_stim_dist = 5.0                                                  # 固視点と刺激の距離 [deg]
...
elemSize=stprm.elemSize                                                        # ドットサイズ
circleArea = np.pi*Disk_radius**2                                 # 円形パッチの面積
elemArea = (elemSize/2)**2 * np.pi                                  # ドットの面積
dotDensity = 300                                                    # ドット密度
Ndots = int(circleArea*(dotDensity/100)/elemArea)                   # ドット数

LeyeStimDot_left = visual.ElementArrayStim(win, units='deg', nElements=Ndots, sizes=elemSize, colors=[black], colorSpace=COLOR_SPACE)
LeyeStimDot_right = visual.ElementArrayStim(win, units='deg', nElements=Ndots, sizes=elemSize, colors=[black], colorSpace=COLOR_SPACE)
ReyeStimDot_left = visual.ElementArrayStim(win, units='deg', nElements=Ndots, sizes=elemSize, colors=[black], colorSpace=COLOR_SPACE)
ReyeStimDot_right = visual.ElementArrayStim(win, units='deg', nElements=Ndots, sizes=elemSize, colors=[black], colorSpace=COLOR_SPACE)

LeyeStimCircle_left = visual.Circle(win, units='deg', radius=Disk_radius, fillColor=white, colorSpace=COLOR_SPACE)
LeyeStimCircle_right = visual.Circle(win, units='deg', radius=Disk_radius, fillColor=white, colorSpace=COLOR_SPACE)
ReyeStimCircle_left = visual.Circle(win, units='deg', radius=Disk_radius, fillColor=white, colorSpace=COLOR_SPACE)
ReyeStimCircle_right = visual.Circle(win, units='deg', radius=Disk_radius, fillColor=white, colorSpace=COLOR_SPACE)
...
for trial in trials:
    ...
    dotPos_left = stim_gene.circlePatch(Ndots, Disk_radius)
    dotPos_right = stim_gene.circlePatch(Ndots, Disk_radius)

    # test刺激が固視点の左に呈示される条件
    if TestPos_LorR == 'left':
        LeyeStimDot_left.setXYs(dotPos_left+LeyeStim_left_centerPos+(disparity_test, 0))
        LeyeStimDot_right.setXYs(dotPos_right+LeyeStim_right_centerPos)
    
        ReyeStimDot_left.setXYs(dotPos_left+ReyeStim_left_centerPos-(disparity_test, 0))
        ReyeStimDot_right.setXYs(dotPos_right+ReyeStim_right_centerPos)

        LeyeStimCircle_left.setPos(LeyeStim_left_centerPos+(disparity_test, 0))
        LeyeStimCircle_right.setPos(LeyeStim_right_centerPos)
        
        ReyeStimCircle_left.setPos(ReyeStim_left_centerPos-(disparity_test, 0))
        ReyeStimCircle_right.setPos(ReyeStim_right_centerPos)


    # test刺激が固視点の右に呈示される条件
    else:
        LeyeStimDot_left.setXYs(dotPos_left+LeyeStim_left_centerPos)
        LeyeStimDot_right.setXYs(dotPos_right+LeyeStim_right_centerPos-(disparity_test, 0))
    
        ReyeStimDot_left.setXYs(dotPos_left+ReyeStim_left_centerPos)
        ReyeStimDot_right.setXYs(dotPos_right+ReyeStim_right_centerPos+(disparity_test, 0))

        LeyeStimCircle_left.setPos(LeyeStim_left_centerPos)
        LeyeStimCircle_right.setPos(LeyeStim_right_centerPos-(disparity_test, 0))
    
        ReyeStimCircle_left.setPos(ReyeStim_left_centerPos)
        ReyeStimCircle_right.setPos(ReyeStim_right_centerPos+(disparity_test, 0))

    # trialに使うコンポーネント
    trialComponents_present = [fp_L, fp_R, 
                               LeyeStimDot_left, LeyeStimDot_right, ReyeStimDot_left, ReyeStimDot_right, 
                               LeyeStimCircle_left, LeyeStimCircle_right, ReyeStimCircle_left, ReyeStimCircle_right, 
    ...
    while routine:
        ...
        if (LeyeStimDot_left.status==NOT_STARTED) and (t_flip >= times['stim'][0]-frame_tolerance):
            print('========呈示開始=======')
            LeyeStimDot_left.status = STARTED                                                              # 呈示開始
            LeyeStimDot_right.status = STARTED                                                             # 呈示開始
            ReyeStimDot_left.status = STARTED                                                              # 呈示開始
            ReyeStimDot_right.status = STARTED                                                             # 呈示開始
            
            LeyeStimCircle_left.status = STARTED                                                              # 呈示開始
            LeyeStimCircle_right.status = STARTED                                                             # 呈示開始
            ReyeStimCircle_left.status = STARTED                                                              # 呈示開始
            ReyeStimCircle_right.status = STARTED                                                             # 呈示開始
            ...
            LeyeStimDot_left.frameNStart = i_frame                                                         # 呈示開始時のフレームを保存
            ...
            LeyeStimCircle_left.draw(win)                                                                     # 刺激を描画
            LeyeStimCircle_right.draw(win)                                                                    # 刺激を描画
            ReyeStimCircle_left.draw(win)                                                                     # 刺激を描画
            ReyeStimCircle_right.draw(win)                                                                    # 刺激を描画
            LeyeStimDot_left.draw(win)                                                                     # 刺激を描画
            LeyeStimDot_right.draw(win)                                                                    # 刺激を描画
            ReyeStimDot_left.draw(win)                                                                     # 刺激を描画
            ReyeStimDot_right.draw(win)                                                                    # 刺激を描画
        
        # 刺激が呈示されているとき
        if LeyeStimDot_left.status == STARTED:
            ...
            if t_flip >= times['stim'][1] - frame_tolerance:
                print('========呈示終了=======')
                LeyeStimDot_left.status = FINISHED                                                         # 呈示終了
                LeyeStimDot_right.status = FINISHED                                                        # 呈示終了
                ReyeStimDot_left.status = FINISHED                                                         # 呈示終了
                ReyeStimDot_right.status = FINISHED                                                        # 呈示終了
                
                LeyeStimCircle_left.status = FINISHED                                                         # 呈示終了
                LeyeStimCircle_right.status = FINISHED                                                        # 呈示終了
                ReyeStimCircle_left.status = FINISHED                                                         # 呈示終了
                ReyeStimCircle_right.status = FINISHED                                                        # 呈示終了
