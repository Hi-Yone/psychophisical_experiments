...
# 円形マスクを作成
radius_pix=stim_gene.deg2pix(Disk_radius)
size=int(radius_pix*2)
radius_mask=radius_pix//2
xx=np.arange(size); yy=np.arange(size)
Y,X=np.meshgrid(xx,yy)
mask = np.sqrt((X-radius_mask)**2 + (Y-radius_mask)**2) <= radius_mask
...
for trial in trials:
    ...
    stimTexture=np.random.uniform(-1, 1, (radius_pix*2, radius_pix*2))
    stimTexture[~mask]=0
    # stimTexture = stim_gene.pix2deg(stimTexture)
    
    # test刺激が固視点の左に呈示される条件
    if TestPos_LorR == 'left':
        # 円形のパッチ刺激
        LeyeStim_left = visual.ImageStim(win, units='pix', image=stimTexture, size=(radius_pix*2, radius_pix*2), pos=LeyeStim_left_centerPos+(disparity_test, 0))
        LeyeStim_right = visual.ImageStim(win, units='pix', image=stimTexture, size=(radius_pix*2, radius_pix*2), pos=LeyeStim_right_centerPos)
    
        ReyeStim_left = visual.ImageStim(win, units='pix', image=stimTexture, size=(radius_pix*2, radius_pix*2), pos=ReyeStim_left_centerPos-(disparity_test, 0))
        ReyeStim_right = visual.ImageStim(win, units='pix', image=stimTexture, size=(radius_pix*2, radius_pix*2), pos=ReyeStim_right_centerPos)

    # test刺激が固視点の右に呈示される条件
    else:
        # 円形のパッチ刺激
        LeyeStim_left = visual.ImageStim(win, units='pix', image=stimTexture, size=(radius_pix*2, radius_pix*2), pos=LeyeStim_left_centerPos)
        LeyeStim_right = visual.ImageStim(win, units='pix', image=stimTexture, size=(radius_pix*2, radius_pix*2), pos=LeyeStim_right_centerPos-(disparity_test, 0))
    
        ReyeStim_left = visual.ImageStim(win, units='pix', image=stimTexture, size=(radius_pix*2, radius_pix*2), pos=ReyeStim_left_centerPos)
        ReyeStim_right = visual.ImageStim(win, units='pix', image=stimTexture, size=(radius_pix*2, radius_pix*2), pos=ReyeStim_right_centerPos+(disparity_test, 0))
