...
surrPatch_range = 13                                # 無相関パッチの描画範囲 [deg]
...
# RefTest_dist = np.array([0, 12])                    # reference刺激とtest刺激の間の距離
...
TestDisk_radius_arr = np.array([0.4, 0.5, 0.7, 0.8, 1, 1.1, 1.3, 1.5, 1.7]) * RefDisk_radius
# TestDisk_radius_arr = np.array([1.541]) * RefDisk_radius
disparity_arr = [0]
...
trialComponents_start = [text_start_L, text_start_R, text_continue_L, text_continue_R, keyboard_start]
trialComponents_resp = [text_resp_L, text_resp_R]
...
for trial in trials:
    ...
    for __ in range(20):
        ...
        corrDots_left, corrDots_right = stim_gene.RefTestDisk(RefDisk_Ndots, TestDisk_Ndots,             # 相関ありのドット刺激を生成
    ...
    print('==============================')
    print(stim_gene.deg2pix(-0.3))
    print(stim_gene.deg2pix(-0.15))
    print(stim_gene.deg2pix(0.0))
    print(stim_gene.deg2pix(0.15))
    print(stim_gene.deg2pix(0.3))
    print(stim_gene.deg2pix(13))
    print('==============================')
    ...
    for thisComponent_start in trialComponents_start:
        if hasattr(thisComponent_start, 'status'):
            thisComponent_start.status = NOT_STARTED
    ...
    for thisComponent_resp in trialComponents_resp:
        if hasattr(thisComponent_resp, 'status'):
            thisComponent_resp.status = NOT_STARTED
    ...
    # テキストの描画を辞める        
    text_resp_L.setAutoDraw(False)
    text_resp_R.setAutoDraw(False)
